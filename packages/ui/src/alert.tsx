  ...
  "relative w-full rounded-lg border p-4 [&>svg~*]:pl-7 [&>svg+div]:translate-y-[-3px] [&>svg]:absolute [&>svg]:left-4 [&>svg]:top-4 [&>svg]:text-foreground",
  {
    variants: {
      variant: {
        ...
        info: "border-blue-200 bg-blue-50 text-blue-700 dark:border-blue-200/30 dark:bg-blue-900/20 dark:text-blue-200",
        success:
...
const AlertTitle = React.forwardRef<HTMLParagraphElement, React.HTMLAttributes<HTMLHeadingElement>>(
  ({ className, ...props }, ref) => (
    <p
      ...
      aria-level={5}
      ref={ref}
      className={cn("mb-1 mt-0 font-medium leading-none tracking-tight", className)}
      {...props}
    />
  )
);
